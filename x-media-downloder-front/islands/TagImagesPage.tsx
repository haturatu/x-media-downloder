...
export default function TagImagesPage(props: TagImagesProps) {
  const { tag, images: initialImages, currentPage: initialCurrentPage, totalPages: initialTotalPages } = props;
  ...
  useEffect(() => {
    if (currentPage !== initialCurrentPage) {
      ...
      fetch(`${API_BASE_URL}/api/images?tags=${encodeURIComponent(tag)}&page=${currentPage}&per_page=100`)
        .then(res => res.json())
        .then(data => {
  ...
  const handlePageChange = (page: number) => {
    setCurrentPage(page);
    window.history.pushState({}, "", `/tags/${tag}?page=${page}`);
  };
  ...
  return (
    <>
      ...
      <div class="p-4">
        <h2 class="text-2xl font-bold mb-4">Images tagged with "{tag}"</h2>
        {loading && <p>Loading images...</p>}
        {error && <p class="text-red-500">Error: {error}</p>}
        {images.length === 0 && !loading && !error && (
          <p class="text-gray-400">No images found for this tag.</p>
        )}
